      ...
      <div className="relative flex overflow-hidden">
        ...
        <div className="flex animate-marquee gap-4 sm:gap-6 pr-4 sm:pr-6">
          {proofBadges.map((badge, index) => (
            ...
            >
              <badge.icon className="w-4 h-4 sm:w-5 sm:h-5 text-primary shrink-0" />
              <span className="text-slate-200 font-bold text-xs sm:text-sm tracking-tight">
                {badge.text}
              </span>
            </div>
          ))}
        ...
        <div
          className="flex animate-marquee gap-4 sm:gap-6 pr-4 sm:pr-6"
          aria-hidden="true"
        >
          {proofBadges.map((badge, index) => (
            <div
